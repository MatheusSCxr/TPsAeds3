...
public class Index_HashExtensivel<T extends models.RegistroHashExtensivel<T>> {

  String nomeArquivoDiretorio;
  String nomeArquivoCestos;
  public RandomAccessFile arqDiretorio;
  public RandomAccessFile arqCestos;
  ...
  public Index_HashExtensivel(Constructor<T> ct, int n, String nd, String nc) throws Exception {
    construtor = ct;
  ...
  public static Index_HashExtensivel<HashElemento> IndexDataBase(Index_HashExtensivel<HashElemento> oldHash, int quant){
    ...
    try (RandomAccessFile database = new RandomAccessFile("./src/resources/db_Output/gamesDB.db","r")) {
      ...
      //excluir os arquivos antigo, se existir
      File file = new File("./src/resources/db_Index/hash_diretorio.db");
      if (file.exists()) {
        //fechar stream e deletar
        if (oldHash != null)
          oldHash.arqDiretorio.close();
        file.delete();
      }
      ...
      if (file2.exists()) {
          //fechar stream e deletar
          if (oldHash != null)
            oldHash.arqCestos.close();
          file2.delete();
      }

      //criar nova hash
      Index_HashExtensivel<HashElemento> hash;
      hash = new Index_HashExtensivel<>(HashElemento.class.getConstructor(), quant, "./src/resources/db_Index/hash_diretorio.db","./src/resources/db_Index/hash_cestos.db");
      ...
      if (database.length() != 0){
          ...
          while (database.getFilePointer() < database.length()){
              try {
                  ...
                  if (lapide != 0xFF){
                      ...
                      //adicionar elemento na Hash
                      hash.create(new HashElemento(jogo.getId(), endereco));
                  }
                  else{
        ...
        try (RandomAccessFile metadados = new RandomAccessFile("./src/resources/db_Index/indexMetadata.db", "rw")){
            DataBase.indexStatus = 2;
            metadados.writeByte(2);
            metadados.writeInt(quant);
        } catch (Exception e){
            System.out.println("[ERRO] Não foi possível escrever os metadados da indexação atual");
        }
      System.out.println("\n[Index] -> Base indexada com sucesso para Hash Extensível em (./src/resources/db_Index/hash_diretorio.db) e (./src/resources/db_Index/hash_cestos.db)");
      }
      else{
          System.out.println("[INFO] -> Não foi detectada um banco de dados em (./src/resources/db_Output/gamesDB.db)");
      }

      //retornar a nova hash
      return hash;

    } catch (Exception e) {
            System.out.println("[ERRO] -> Não foi possível indexar o banco de dados para Hash Extensível");
            System.out.println(e);
    }
