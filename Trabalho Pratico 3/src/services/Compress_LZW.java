 ...
 *  Agora ela é capaz de codificar/decoficar uma sequência de bytes
 *  não estando limitada apenas a strings.
**/


public class Compress_LZW {

    public static final int BITS_POR_INDICE = 12; // Mínimo de 9 bits por índice (512 itens no dicionário)


    //método experimental mais eficiente para leitura completa da base de dados. Menos operações de I/O
    public static void compress_DataBase() throws Exception{
            //excluir o arquivo antigo, se existir
            File file = new File("./src/resources/db_compress/Compressed_DB.db");
            if (file.exists()) {
                file.delete();
            }

            RandomAccessFile compressed = new RandomAccessFile(file, "rw");
            
            RandomAccessFile database = new RandomAccessFile("./src/resources/db_Output/gamesDB.db","r");
            

            //criar dicionário inicial
            ArrayList<ArrayList<Byte>> dicionario = new ArrayList<>(); //dicionário, de vetores bytes
            ArrayList<Byte> vetorBytes; // elemento (vetor de bytes) para inclusão no dicionário
            byte b;
           
            //preencher dicionário inicial
            for (int j = -128; j < 128; j++) { // Usamos uma variável int para o laço, pois, em uma variável byte, 127 + 1 == -128
                b = (byte)j;
                ...
                while (database.getFilePointer() < database.length()){
                    try {
                        ...
                        if (lapide != 0xFF){
                            ...
                            //obter o tamanho do registro em bytes
                            int tam_elemento = database.readInt();

                            //array de bytes com o tamanho exato do elemento
                            byte[] dadosElemento = new byte[tam_elemento];

                            //obter todos os bytes do elemento de uma vez
                            database.readFully(dadosElemento);
                            
                            //comprimir
                            compressed.write(codifica(dadosElemento,dicionario));
                        }
                        else{
                        ...
                        conta++;
                        UI.progressBar(conta, DataBase.totalGames + DataBase.totalDeleted, "[Compress]", 8, 0);
                    } catch (Exception e) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            byte[] msgCodificada = codifica(msgBytes,dicionario); // Vetor de bits que contém os índices
    ...
    public static byte[] codifica(byte[] msgBytes, ArrayList<ArrayList<Byte>> dicionario) throws Exception {
